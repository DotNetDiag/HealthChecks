...
public class activemq_healthcheck_should
{
    [Fact]
    public async Task be_healthy_when_connection_and_session_can_be_created()
    {
        ...
        var healthCheck = new ActiveMQHealthCheck(
            connectionFactory,
    ...
    public async Task use_credentials_when_user_name_or_password_is_configured()
    {
        ...
        var connection = Substitute.For<IConnection>();
        var session = Substitute.For<ISession>();
        connectionFactory.CreateConnection("user", "password").Returns(connection);
        ...
        result.Status.ShouldBe(HealthStatus.Healthy);
        connectionFactory.Received(1).CreateConnection("user", "password");
    ...
    public async Task be_unhealthy_when_connection_can_not_be_created()
    {
        ...
        var healthCheck = new ActiveMQHealthCheck(connectionFactory);
    ...
    {
        Action action = () => new ActiveMQHealthCheck(null!);
    ...
    private static HealthCheckContext CreateContext(HealthStatus failureStatus = HealthStatus.Unhealthy) =>
        new()
        {
            Registration = new HealthCheckRegistration("activemq", _ => Substitute.For<IHealthCheck>(), failureStatus, [])
        };
}
